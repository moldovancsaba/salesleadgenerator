// Issue #195/#196 — pure, zero-I/O brand types/constants, split out of
// app/lib/brand.ts specifically so a Client Component can import a plain
// value like CURRENCY_CODE_OPTIONS/CURRENCY_SYMBOLS without pulling in that
// file's Mongo-backed accessors (and therefore lib/mongodb.ts's `mongodb`
// driver, which needs Node core modules like `net`/`tls` that don't exist
// in a browser bundle — confirmed as a real webpack "Module not found: net"
// build failure before this split existed). app/lib/brand.ts re-exports
// everything here, so every existing server-side import of a type/constant
// from '.../brand' keeps working unchanged; only a Client Component
// importing a *value* (not just a type, which TypeScript erases at compile
// time regardless of which file it's imported from) needs to import it from
// here instead.
...
export interface BrandConfig {
  ...
  // getForbiddenTermsFor() (app/lib/brand.ts) unions every *other* brand's
  // list to build the actual forbidden set for a given brand, so this never
  // needs manual symmetric upkeep the way the old FORBIDDEN_BRAND_TERMS map did.
  ownNameTerms: string[];
...
// (app/lib/brand.ts) returns when the `brands` collection is genuinely
// empty (a fresh environment before the migration script has run). Once
// `brands` holds at least one document, MongoDB is fully authoritative — a
// slug not found there is `null`, never silently patched from here. Never a
// second, coexisting source of truth once brands exist in Mongo.
//
