...
export default function SeyuSalesPage() {
  ...
  async function fetchLeads() {
    try {
      const response = await fetch("/api/seyusales/leads");
  ...
  async function handleMove(leadId: string, column: KanbanColumn, sortOrder: number) {
    ...
    try {
      const response = await fetch(`/api/seyusales/leads?id=${leadId}`, {
        method: "PATCH",
  ...
  async function handleAction(leadId: string, action: string, payload: any) {
    try {
      await fetch(`/api/seyusales/leads?id=${leadId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
