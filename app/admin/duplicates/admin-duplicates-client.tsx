...
export function AdminDuplicatesClient() {
  ...
  const runScan = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json()
      // Issue #107: the scan is capped (5000 leads since issue #137) for perf reasons — when
      ...
      const truncationNote = data.truncated
        ...
        : ''
      showNotification({
        message: `Scanned ${data.scanned} leads${truncationNote} — ${data.newPairs} new candidate pair${data.newPairs === 1 ? '' : 's'} found, ${data.unresolvedPairs} awaiting a decision.`,
