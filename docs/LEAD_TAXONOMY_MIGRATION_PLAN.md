...
- **Issue #136 — RESOLVED** (decision rule codified in `docs/LEAD_ENRICHMENT_GUIDE.md` §2.6): if a confirmed separate legal entity organizes the event, use `competition-organiser`; if the event has no confirmed separate identity from its federation, use the federation's own `orgTypeCode` (`federation`); never default to `tournament` for this shape. Retroactively applied to the 2 leads issue #136 itself had already researched and named: ICC Cricket World Cup (`tournament` → `federation`, no separate legal entity confirmed) and Rugby World Cup (`tournament` → `competition-organiser`, confirmed separate legal entity "Rugby World Cup Limited"). Commonwealth Games (`federation`) and Billie Jean King Cup (`competition-organiser`) already matched the rule — no change needed. The Hundred's `league` classification is a different, unrelated code (round-robin domestic season, not part of this three-way ambiguity) — untouched.
  **Not yet retroactively fixed, real work remaining**: FIFA World Cup (batch 2.4.137), IHF World Handball Championship (batch 2.4.145), FIVB Volleyball World Championship (batch 2.4.149), and ICC T20 World Cup (batch 2.4.154) are all currently `tournament` but were never individually researched for separate-legal-entity status the way ICC Cricket World Cup and Rugby World Cup were — applying the now-codified rule to these 4 requires the same per-lead legal-entity research, not a mechanical reclassification. Left as `tournament` (a valid, honest value) rather than guessed at.
