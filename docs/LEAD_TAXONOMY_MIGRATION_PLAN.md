...
## 9. Session handoff — resuming the agent-research classification loop (as of 2026-07-31, v2.4.154)
...
### Owner decisions received 2026-08-01 — resolved, with remaining retroactive-fix scope

- **Issue #135 — RESOLVED.** Owner decision: `orgTypeCode: "brand"` is now the standing convention for platform/tech-brand leads with no clean taxonomy fit (documented in `docs/LEAD_ENRICHMENT_GUIDE.md` §2.6). Retroactively applied to both known data points: Strava (was `unknown`) and YouTube (was `media`). Any future lead in this shape should be classified `brand` directly, not flagged as an open question again.
- **Issue #136 — RESOLVED** (decision rule codified in `docs/LEAD_ENRICHMENT_GUIDE.md` §2.6): if a confirmed separate legal entity organizes the event, use `competition-organiser`; if the event has no confirmed separate identity from its federation, use the federation's own `orgTypeCode` (`federation`); never default to `tournament` for this shape. Retroactively applied to the 2 leads issue #136 itself had already researched and named: ICC Cricket World Cup (`tournament` → `federation`, no separate legal entity confirmed) and Rugby World Cup (`tournament` → `competition-organiser`, confirmed separate legal entity "Rugby World Cup Limited"). Commonwealth Games (`federation`) and Billie Jean King Cup (`competition-organiser`) already matched the rule — no change needed. The Hundred's `league` classification is a different, unrelated code (round-robin domestic season, not part of this three-way ambiguity) — untouched.
  **Remaining 4 resolved 2026-08-02 (issue #163)**, each independently researched via real live sources (governance statutes, official contracts, commercial-registry lookups where relevant — full citations in #163's comment thread): FIFA World Cup → `federation` (FIFA's own 2021 annual report and 2026-edition reporting confirm no genuinely independent legal entity — "FIFA World Cup Qatar 2022 LLC" was a FIFA-controlled subsidiary, not a separate operating body; FIFA dispensed with even the local-organizing-committee model for 2026, running host-city contracts directly). IHF World Handball Championship → `federation` (IHF's own Statutes and Standard Contract for IHF Competitions state IHF is the event holder and rights-owner, merely transferring per-edition hosting to the host federation's Local Organising Committee, which stays legally subordinate to IHF; "essentially an IHF event" per the contract's own text). ICC T20 World Cup → `federation` (same structure as its sibling ICC Cricket World Cup lead — ICC runs it directly; ICC Business Corporation FZ LLC is a wholly-owned commercial subsidiary, not an independently governed organizer). **FIVB Volleyball World Championship → `competition-organiser`** (the one case of this batch matching the Rugby World Cup pattern: FIVB partnered with CVC Capital Partners in 2021 to form Volleyball World, operated through a separately-incorporated Swiss company, VW Volleyball World SA (Lausanne trade-register no. CHE-334.506.517) — confirmed via a Swiss commercial-registry lookup, a genuinely independent legal entity, not a FIVB-owned instrumentality).
- **Issue #143 — RESOLVED.** Owner decision: yes, Seyu's fan-engagement/sponsor-activation product genuinely targets non-sport entertainment properties. `ORG_TYPE_CODES` extended with a new `entertainment-event` value (`lib/lead-taxonomy.ts`, documented in `docs/LEAD_ENRICHMENT_GUIDE.md` §2.6). Retroactively applied to both known data points: Tomorrowland and Glastonbury Festival (both were `event-organiser`/`not-applicable` stretch-fits, now `entertainment-event`). `sportCode`/`genderCode`/`competitionLevelCode` remain `not-applicable` for these — that part of the original classification was already correct.
...
See also: `docs/ARCHITECTURE.md`'s "Controlled Sports-Industry Taxonomy" section (the schema this plan backfills), `docs/LEAD_ENRICHMENT_GUIDE.md` (the classification/enrichment prompt this plan's execution reuses — §5's fenced block is the literal text to give each research agent), `docs/LESSONS_LEARNED.md` (the general pattern of dry-run-first validation this plan follows, and the real incidents — e.g. the 2.4.106→2.4.107 matching-criteria correction — that motivate it), `CHANGELOG.md` (2.4.107/2.4.108 entries, the most directly analogous prior large-scale data operation in this repo's history; 2.4.121 onward for this loop's own real-time history), GitHub issue #132 (the tracking issue with the fullest real-time progress history), issues #135/#136/#143 (taxonomy-vocabulary questions this loop surfaced, resolved by the owner 2026-08-01 — see this section's "Owner decisions received" note above for what's still outstanding on #136's retroactive-fix scope).
