...
## 9. Session handoff — resuming the agent-research classification loop (as of 2026-08-01, v2.4.160)
...
**Progress as of this checkpoint:** **123 of ~2,723 leads have full taxonomy** (`orgTypeCode` set, meaning the classification pass genuinely ran on them — either with real evidence or with an honest `orgTypeCode: "unknown"` after a real search found nothing). Separately, **2,424 of 2,723 leads have `sportCode` set** from the earlier mechanical backfill (§2/§4 above) — that's a much larger number but is *not* the same thing as full classification; those leads still need this same evidence-based pass for every other field. ~2,677 leads remain untouched by this loop. At the proven pace (~4 leads fully classified per ~15-20 minute batch cycle, run continuously), this is a genuinely large, multi-session undertaking — budget accordingly, don't assume it finishes in one sitting.
...
- **Issue #136**: a recurring "is this a tournament, league, or the federation/organiser that runs it" ambiguity for major global sports properties with no clean separate legal identity — 13 leads so far have landed on 5 different `orgTypeCode` answers (`tournament`, `federation`, `competition-organiser`, `league` for The Hundred and the Indian Premier League, both reasoned as recurring domestic round-robin competitions with fixed franchise teams rather than a single knockout event, and now `event-organiser`). The FIFA World Cup, the IHF World Handball Championship, the FIVB Volleyball World Championship, and the ICC T20 World Cup are all reasoned to `tournament` (grouped as quadrennial/biennial knockout-style mega-competitions). The Six Nations Championship (batch 2.4.159) is a genuinely new shape for this series: a recurring ANNUAL round-robin among a FIXED set of 6 *national representative teams* (not domestic clubs, not a rotating-host knockout event) — reasoned to `competition-organiser` as the closest existing controlled value for the commercial body that runs it, since neither `tournament` nor `league` cleanly fits this national-teams-round-robin shape. **The Asian Games (batch 2.4.160) is a third genuinely new shape**: a quadrennial MULTI-SPORT games event (a regional Olympics, spanning dozens of sports at once) run by a permanent continental confederation (the Olympic Council of Asia, 45 Asian National Olympic Committees) — it fits none of the prior codes (not single-sport like the `tournament` group, no fixed franchises/round-robin like `league`, not an annual single-sport national-teams round-robin like the Six Nations' `competition-organiser`) — reasoned to `event-organiser` as the closest fit for a body whose defining activity is producing large-scale, recurring multi-sport Games events. Still not resolved; needs owner adjudication across all data points collected so far.
