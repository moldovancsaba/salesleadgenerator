...
## 9. Session handoff — resuming the agent-research classification loop (as of 2026-07-31, v2.4.136)
...
- **Issue #136**: a recurring "is this a tournament, league, or the federation/organiser that runs it" ambiguity for major global sports properties with no clean separate legal identity — 7 leads so far have landed on 4 different `orgTypeCode` answers (`tournament`, `federation`, `competition-organiser`, and now `league` for The Hundred, batch 2.4.136 — reasoned as a round-robin/table-based domestic season rather than a single knockout event) via genuinely defensible but inconsistent reasoning each time. Still not resolved; needs owner adjudication across all data points collected so far.
