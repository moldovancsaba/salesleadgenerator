...
### Kanban Lead Card
As of 2.4.17, `app/card.tsx`'s `LeadCard` renders **flat, borderless** content (plain Mantine `Stack`/`Group`/`Text`/`Badge`/`Button` — no `ProductCard`, no `Paper`/`Card` wrapper of its own). This replaced a `ProductCard`-based version (used 2.3.1–2.4.16) after GDS's own `KanbanCard` (see below) turned out to *always* wrap whatever `renderItem` returns in its own bordered `Paper` — nesting `ProductCard`'s own `withBorder` shell inside that produced a visible "box within a box" around every card. `LeadCard` is only ever used inside the kanban board's `renderItem`, so GDS's `KanbanCard` border is now the only border rendered. Cards show a "ticket size" row (`getTicketSize()` in `app/constants.ts`) — the estimated deal value, direct for CogMap (`estimated_annual_revenue_usd`), summed from per-lead pricing blocks for Seyu (`pricingByCompany`).
...
**`enableDrag` is deliberately off (as of 2.4.17)**, reverting a 2.4.10–2.4.16 attempt to enable it. `enableDrag` activates GDS's real `@dnd-kit`-based pointer/touch/keyboard drag (fully encapsulated inside `gds-core`, never a direct app import) plus a per-card drag-handle icon — but it was also the one genuinely new runtime code path in the whole GDS 3.11.x adoption that had never actually executed in a successful production build (every earlier deploy attempt from this bump failed before reaching it) before a live "client-side exception" was reported. Disabling it is a disclosed, reasoned rollback pending real root-cause confirmation, not a proven fix — the keyboard/tap "Move to column" menu is unconditional and unaffected, so moving cards between columns still works fully without pointer drag. If `enableDrag` is re-enabled in the future, `onMoveItem`'s same-column no-op guard (below) becomes relevant again: GDS's `SortableContext` allows same-column reordering, which this app's PATCH API has no way to represent (no arbitrary drop-position concept; `sortOrder` is ignored entirely for the two ICE-sorted auto-managed columns) — `handleMoveItem` already explicitly no-ops that case.
