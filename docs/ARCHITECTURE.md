...
Rendered in two places: `app/card.tsx` shows the nudge `message` as decorative `Text` only (no button, no `role="button"`/`tabIndex` — matches the card's existing "click Preview to act" pattern) colored orange for `severity: 'warn'`, dimmed for `'info'`. `app/detail.tsx` renders the same message plus, only when `nudge.actionable && nudge.action === 'REQUEST_REFRESH'`, a real `Button` wired to the modal's existing `handleRefresh()` (the same handler the icon-only ActionBar refresh action already uses) — reusing existing PATCH/notification logic rather than duplicating it. The modal computes `staleness`/`nudge` from `DEFAULT_STALE_THRESHOLDS` rather than a brand-fetched `/api/settings` value, since the modal makes no additional network calls by design (mirrors `app/kanban.tsx`'s own fallback-to-defaults behavior on fetch failure).
