...
As of 2.4.10, `app/kanban.tsx` renders via GDS 3.11.0's governed `KanbanBoard` (`@sovereignsquad/gds-core/client`, `enableDrag`) instead of a hand-rolled implementation. GDS handles layout (`useGdsKanbanOrientation` resolves stacked-vs-columns responsively, no app-level breakpoint logic needed), drag-and-drop (`@dnd-kit`-based pointer/touch/keyboard drag, fully encapsulated inside `gds-core` — never a direct app import), and an unconditionally-rendered keyboard-accessible "Move to column" menu per card as the accessible fallback. Each lead is mapped to a `KanbanItem`/`KanbanColumnData` pair; `renderItem` renders the existing `LeadCard` (`ProductCard`-based) inside GDS's governed card shell. Two API constraints shaped the mapping: `KanbanColumnData.title` is a plain string (not `ReactNode`), so the per-column pipeline-weighted forecast (sourced from `GET /api/boards/[brand]`'s `forecast.pipeline[COLUMN]`) is encoded into the title as one line (e.g. `"Discovered (12) · $45,231 wtd"`) rather than a separate styled subtitle; `KanbanColumn` has no footer/pagination slot, so the cursor-based infinite-scroll "load more" sentinel is rendered inside the last card's `renderItem` output instead of as a column-level sibling. `onMoveItem` wraps the existing `handleMove()` (optimistic removal from the source column, reload both columns from the server), explicitly no-op'ing a same-column drag — GDS's `SortableContext` allows same-column reordering, which this app's PATCH API has no way to represent (no arbitrary drop-position concept; `sortOrder` is ignored entirely for the two ICE-sorted auto-managed columns).
