...
- **Saved filters**: with a region, industry, and/or tags set, tap **Save filter** and give it a name — it appears as a pill you can tap to re-apply later. Saved to your account, scoped per brand — it follows you to any device or browser you sign into. Saving under a name you've already used replaces that saved filter rather than creating a duplicate. Capped at 20 per person per brand; the oldest is dropped automatically if you go over.
  - **Sharing (brand admins only)**: check **Share with team** before saving to publish that view to everyone with access to the brand — they see it read-only (apply only, no edit/delete), attributed with your email. A shared filter you own can be un-shared any time from its share toggle next to the pill.
  - **First time after this shipped**: if you had saved filters from before (stored only in that one browser), a one-time banner offers to **Import** them into your account so they follow you everywhere going forward — dismissing it leaves your local copy untouched, so you can import later if you change your mind.
...
- **Edit Lead Details**: an Edit/Save/Cancel form for `entity_name`, `url`, `country` (ISO country code such as `ES`, `GB`, `US`; since 2.4.234 only real codes are accepted, and an unchanged country is not re-sent, so a lead with an old bad code can still save its other fields), `address`, `general_contact`, `size`, `industry`, `sport_or_sector`, `level_league`, `value_proposition`, `notes`, `tags`. This form does **not** yet include the newer controlled-taxonomy fields (`sportCode`, `orgTypeCode`, `businessUnitCode`, etc., added 2.4.109) — those are API-only for now; see [Lead Taxonomy](#lead-taxonomy) and [Known Issues and Limitations](#known-issues-and-limitations).
...
Click **Scan for duplicates** for a brand to compare leads by name similarity — gated on matching activity (a lead's controlled `sportCode` when set, 2.4.109, otherwise its free-text `sport_or_sector`; two leads with different or unknown sports/sectors are never flagged, even with identical names) — and file new candidate pairs for review (capped at the 5000 most recent leads per scan — if the brand has more, a notification says the scan was truncated). The notification after each scan also says how many candidate pairs are still awaiting a decision; re-scan after a review session to see that number fall. Domain match is shown as extra context on a flagged pair, never a reason to flag one by itself. Use the **Status** switcher (Pending / Confirmed / Dismissed / Merged) to move between stages — a pair leaves the Pending list the moment you decide on it, so Confirmed is where you'll find pairs waiting to be merged.
...
**Auth:** every lead endpoint below (`GET`/`POST`/`PATCH /api/leads`, `GET /api/leads/columns`, `PATCH /api/leads/bulk`, `GET`/`DELETE /api/leads/[id]`) requires either an `x-api-key` header (shown below) or an authenticated browser session with access to the requested `brand`. There is no unauthenticated read path. The same session-or-key rule covers Sales Settings (`/api/sales-settings/[brand]`) and the product catalog (`/api/products/[brand]`) since 2.4.226 (issue #226; both were open before). `GET /api/outcome-logs` and `GET /api/outreach-logs` need `x-api-key`; nothing in the app calls them. Since 2.4.227 (issue #227) outreach send/log, templates, cadences, automation rules and battlecards also take the session-or-key rule; they need `?brand=` (for battlecards it is required, not defaulted). Before that their saves needed `x-api-key`, so they failed with 401 for every browser user. `POST /api/leads` previously required `x-api-key` exclusively (only the research agent ever called it); as of 2.4.96 it accepts a browser session too, so the in-app **Add Lead** button (see [Add Lead](#add-lead)) can call it directly — the `x-api-key` path is unchanged for existing callers. **`PUT /api/leads/[id]` is the one exception**: it requires an API key, never a session — it's the research agent's own enrichment path and has no in-app browser caller (corrected during the 2026-07-27 documentation audit; previously grouped in with the session-accepting endpoints above). It accepts the shared key or, since 2.4.232 (issue #220), a read-write scoped key for that brand from `/admin/api-keys`, so the research agent can move to per-brand keys. Every request that still uses the shared key is counted on `/admin/api-keys` under "Shared key usage".
...
### Outreach Analytics
```bash
curl "https://salesleadgenerator.vercel.app/api/outreach-templates?brand=cogmap&mode=analytics" \
  -H "x-api-key: YOUR_API_KEY"
```
