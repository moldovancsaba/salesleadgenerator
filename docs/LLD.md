...
| `app/api/leads/[id]/route.ts` | GET, PUT, DELETE | PUT: `requireMachineKeyApi` (shared or brand-scoped read-write key, never a session; issue #220, 2.4.232); GET/DELETE: `requireBrandAccessApi` | Full read/replace/delete of one lead; PUT re-derives kanban column, dedupes contacts, kicks off async `verifyLeadContactsAsync` + `computeTicketSizeForLead` |
...
| `app/api/admin/api-keys/legacy-usage/route.ts` | GET | `requireSuperAdminSession` | Issue #220: shared-key (`SLG_API_KEY`) usage over the last `?days=` (≤120), grouped by method and route, shown on `/admin/api-keys` |
...
| `legacy_api_key_usage` | `{day, method, path, count, firstSeenAt, lastSeenAt, lastUserAgent}`, unique `{day, method, path}`, TTL 120 days on `firstSeenAt` | `lib/legacy-key-usage.ts` | written fire-and-forget by every helper that accepts the shared key (`requireApiKey`, `requireBrandAccessApi`, `requireApiKeyOrSession`, `requireMachineKeyApi`); ids in paths collapse to `:id` (issue #220) |
