...
| `app/api/integrations/oauth/callback/route.ts` | GET | none (PKCE `state`/`integ_oauth_verifier` cookie check, not a session guard) | Exchanges the OAuth code, `upsertOAuthConnection`, redirects back to the integrations settings page |
...
| `quote_view_rate_limits` | one doc per view request, TTL | `app/lib/quotes-store.ts` | `GET /api/quotes/[quoteId]/view`, 20/min per `quoteId` (issue #211) |
