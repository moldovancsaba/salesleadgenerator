...
**Discovered, pre-existing, unrelated gap — not fixed here, flagged for awareness**: `POST /api/leads`'s own `newLead` document construction never actually persists the validated `country` field (it's used for validation/fingerprinting but silently dropped before insert) — confirmed by reading `app/api/leads/route.ts` and reproduced identically by this import (which intentionally mirrored that exact logic for consistency). `region`/`address` still carry most of the same geographic signal. Worth a small, separately-scoped fix if `country`-level filtering/reporting ever matters for these or any other leads.
