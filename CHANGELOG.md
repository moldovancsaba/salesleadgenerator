...
The actual payoff of 2.4.185's Mongo-backed brand registry: a super admin can now create a new client/brand entirely through the app, no code deploy. `app/admin/clients/page.tsx` + `admin-clients-client.tsx` (an `AdminDataTable` list plus an inline create form, same session gate and structural pattern as the existing `/admin/users`/outreach-templates admin pages) and `GET`/`POST /api/admin/clients`. The form's fields map directly onto a `BrandRecord`: slug, label, currency, aliases, own-name/competitor terms, forecast model (defaults to the generic model, with an explicit warning for the "custom" option), optional sales-vocabulary picks and outreach from-address — server-derives `dbCollection` (`${slug}_leads`) and `apiPrefix` (always `/api/leads`), neither client-settable. Uniqueness enforced twice: an application-level pre-check for a fast `409`, and the real guarantee is `brands`'s own unique indexes (from 2.4.185), with a caught `E11000` returning `409` for the genuine concurrent-create race.

**A real, disclosed bug found and fixed during this work, already merged as part of this same change:** `app/lib/brand.ts` (2.4.185) imports `lib/mongodb.ts` at module scope. A production `next build` (not caught by `tsc`/`lint`, neither of which check bundling boundaries) found that any Client Component importing a real runtime *value* from it — not just a `type`, which TypeScript erases entirely regardless of source file — pulls the `mongodb` driver into the browser bundle, which needs Node core modules (`net`, `tls`, `dns`, `fs`, …) that don't exist there. This was **already broken on `main` for `/salessettings/[client]`** from the 2.4.185 merge itself (confirmed via `next build`), not something newly introduced here — it was invisible until this change's own new admin page hit the same pattern and a real build was actually run. Fixed by splitting the pure, zero-I/O types/constants into a new `app/lib/brand-constants.ts` (re-exported by `app/lib/brand.ts`, so every existing server-side import keeps working unchanged); the handful of files needing an actual value (`admin-clients-client.tsx`, `sales-settings-client.tsx`, `app/lib/sales-settings.ts`, `lib/deals.ts`, `app/detail.tsx`, `app/lib/normalize-lead.ts`, `lib/validate-lead.ts`) now import it from `brand-constants` instead. `app/lib/brand.ts` gains a real new dependency, `server-only`, so an accidental future client-value-import fails with a clear, direct error instead of a confusing "Module not found: net" trace. `server-only` throws unconditionally outside Next.js's own build (which aliases it to a no-op server-side), which broke direct unit/integration tests of modules importing it — fixed by aliasing it to a local no-op stub (`tests/helpers/server-only-noop.ts`) in both Vitest configs, matching Next.js's own behavior.

New `tests/integration/admin-clients.integration.test.ts` (15 tests, real `mongodb-memory-server`) covers auth gating (401/403, session mocked per the `duplicate-review-merge.integration.test.ts` precedent — a real signed SSO JWT can't be fabricated in this sandbox), slug/label/currency validation, successful creation with derived defaults, and both real uniqueness-collision shapes (duplicate slug, alias colliding with another brand's slug or alias). Verified end-to-end via a full `next build` (every route compiles) plus manual `curl`/browser checks of the unauthenticated-redirect path (can't test the authenticated create-flow interactively without a real SSO session, same disclosed limitation as every other admin feature in this app).

`docs/OPERATOR_GUIDE.md` gains a "Clients" admin-tools section; `docs/ARCHITECTURE.md` documents both the feature and the bundling-boundary fix in full.

### Testing
`npx tsc --noEmit` — 0 errors. `npm run lint` — 0 errors/warnings. `npx vitest run` — 735/735 passing. `npm run test:integration` — 227 passing across 26 files (was 212/25; +15 new in `tests/integration/admin-clients.integration.test.ts`). `npm run test:smoke` — 5/5 passing. `npx next build` — compiles clean for all 27 routes (the check that actually caught the bundling bug above). `npm run audit:gds-style` — 25 findings, 24 confirmed pre-existing/unrelated on `main` before this change; the 1 new one (`tests/integration/brand.integration.test.ts`) is the same class of false positive as the other 24 (a 6-hex-character substring — MongoDB's own `E11000` duplicate-key error code, asserted verbatim in a test — matched by the audit's color-literal pattern with no `#` present).

## 2.4.185

### Brand config becomes Mongo-backed (issue #195)

Prerequisite backend for a site-admin "add a new client" feature (issue #196, tracked separately): moves brand/tenant configuration — `app/lib/brand.ts`'s `BRAND_CONFIG`, `lib/validate-lead.ts`'s `FORBIDDEN_BRAND_TERMS`, and `app/lib/forecast.ts`'s per-brand forecast routing — from a static TypeScript object (requiring a developer to hand-edit source and deploy for every new brand) into a new `brands` Mongo collection, read at request time. No admin UI ships in this change; that's issue #196.

`app/lib/brand.ts` gains async `getBrandConfig()`/`getAllBrandConfigs()`/`resolveBrand()`, reading `brands` fresh on every call with no in-memory cache (no such pattern exists anywhere in this codebase, and it would be unreliable on Vercel's serverless model anyway) — same "read fresh, code-constant fallback" shape already used for `settings`/`pipeline_weights`. `Brand` is now `type Brand = string` rather than a 3-value literal union, since a runtime-editable set of brands can't be a compile-time union. The old `BRAND_CONFIG` object is retained, renamed `FALLBACK_BRAND_CONFIG`, used only as migration seed data and as the return value when `brands` is genuinely empty — once populated, Mongo is fully authoritative.

`getForbiddenTermsFor(brand)` replaces the old hand-maintained, must-stay-symmetric `FORBIDDEN_BRAND_TERMS` map with a derived union of every *other* brand's own `ownNameTerms` — this incidentally **fixes a real, pre-existing gap**: DVSC's old hand list was missing 6 terms (`sports science`, `situational awareness`, `jumbotron`, `sponsor activation`, `revenue-share`, `revenue share`) that CogMap's and Seyu's lists already protected against; the derivation now includes them correctly, not preserved as a silent asymmetry. `app/lib/forecast.ts`'s hardcoded `if (brand === 'cogmap' || brand === 'dvsc')` forecast-model gate becomes a per-brand `forecastModel: 'dealSizeBand' | 'custom'` flag — safe because `computeDealSizeBandForecast()` was already confirmed fully generic (reads only generic Lead-document fields, no brand-name-specific logic), so any new brand defaulting to `'dealSizeBand'` reuses it with zero code change; Seyu's genuinely bespoke `pricingByCompany` model stays hardcoded, `forecastModel: 'custom'`.

`lib/outreach-send.ts`'s per-brand outreach sender moves from a `RESEND_FROM_<BRAND>` env var to the brand's own `fromEmail` field — no more Vercel dashboard edit + redeploy per brand (see `docs/STACK_AND_DEPENDENCIES.md`).

**Call-site sweep**: a grep-verified 51 files imported `BRAND_CONFIG` before this change. Most needed a mechanical `await`; two modules with functions deliberately kept synchronous/DB-free (`lib/sso-access.ts`, `app/lib/inbound-email.ts`, so their existing unit tests never needed a database) instead gained an explicit `allBrands` parameter supplied by their one real caller per request, preserving that; brand-scoped Client Components now receive `label`/`currency`/`salesVocabulary` as props from their already-brand-resolving parent Server Component instead of importing brand config directly (a browser bundle can't call the Mongo-backed accessors); `AppNav`/`admin-users-client`/`admin-duplicates-client` read a new `brandLabels` field on `GET /api/auth/session`'s response instead of a stale module-scope constant. Full detail in `docs/ARCHITECTURE.md`'s "Brand config becomes Mongo-backed" section.

New `scripts/migrate-brands-to-mongo.ts` (one-time, manual, idempotent — not run against production from this environment, no live Mongo access here) seeds `brands` with cogmap/seyu/dvsc's real current values. New `tests/integration/brand.integration.test.ts` covers the live Mongo-backed accessor behavior (fallback-when-empty, Mongo-becomes-authoritative-once-populated, the slug/alias uniqueness index, `getForbiddenTermsFor`'s derivation) against a real `mongodb-memory-server` instance. `tests/lib/brand.test.ts`, `tests/lib/sso-access.test.ts`, `tests/lib/inbound-email.test.ts`, `tests/lib/sales-settings.test.ts`, `tests/lib/validate-lead.test.ts`, `tests/lib/validate-battlecard.test.ts`, `tests/lib/field-verifications.test.ts`, `tests/lib/outreach-send.test.ts`, and `tests/smoke/validate-lead.smoke.ts` updated for the new signatures. Also fixed a genuine, newly-surfaced test-infrastructure bug while verifying: `tests/integration/outreach-send.integration.test.ts` statically imported `sendAutomatedEmail` at module top, which now transitively depends on `lib/mongodb.ts`'s module-load-time `MONGODB_URI` singleton — a static import resolved that chain before `startTestMongo()` ever set the URI, permanently caching an unconfigured client for the file. Fixed by dynamically importing it in `beforeAll`, after the URI is set, the same pattern every route-handler integration test in this repo already uses for exactly this reason.

Filed as two GitHub issues per this repo's own issue-driven workflow: [#195](https://github.com/moldovancsaba/salesleadgenerator/issues/195) (this backend migration) and [#196](https://github.com/moldovancsaba/salesleadgenerator/issues/196) (the site-admin `/admin/clients` UI/API built on top of it, not yet implemented).

### Testing
`npx tsc --noEmit` — 0 errors. `npm run lint` — 0 errors/warnings. `npx vitest run` — 735 passing (was 728; +7 net across `tests/lib/brand.test.ts`'s new `getBrandConfig`/`getAllBrandConfigs`/`getForbiddenTermsFor` coverage). `npm run test:integration` — 212 passing across 25 files (was 201/24; +11 new in `tests/integration/brand.integration.test.ts`). `npm run test:smoke` — 5/5 passing. `npm run audit:gds-style` — 24 pre-existing findings, confirmed identical on `main` before this change (false-positive hex-like substrings in test emails/URLs and `app/layout.tsx`'s theme-color meta tag) — not introduced or touched by this change.
...
Grepped every `.region` call site. One (`app/detail.tsx`'s badge-tone ternary) assumed the 3-value union without breaking on other input — it already had a graceful `'gray'` fallback. Two more (`app/components/AddLeadModal.tsx`'s creation form, `app/components/FilterBar.tsx`'s filter drawer) hardcoded the same 3-value `Select` — a real, disclosed gap the type widening alone didn't fix: an operator could not create or filter a lead with any of the 55+ real observed values. Fixed both by converting from `Select` to free-text `TextInput` (uppercased on change), matching the exact pattern the adjacent `country` field already uses in both components. Also removed an unrelated, genuinely unused `REGIONS = ['US', 'CEE', 'MENA']` constant in `app/api/metrics/route.ts` (declared, never referenced), found while investigating call sites — it was misleadingly implying only 3 regions exist.
...
Following up on Rule 8's expansion (2.4.172), performed the retroactive sweep it calls for: found and stripped the auto-appended `_Generated by [Claude Code](...)_` footer from 15 merged PR descriptions (#153–#174) and an older body-embedded `Co-Authored-By`/`Claude-Session` trailer from PR #37 — all engineering content preserved verbatim, only the attribution removed. Re-confirmed via repo-wide search: zero remaining matches.

Investigated all 22 non-`main` branches for staleness. 17 confirmed safe to delete (16 via a real merged-PR record, 1 via literal git ancestry) but **branch deletion is not reachable from this session** — `git push origin --delete` returns a genuine `403` straight from git, and no GitHub MCP tool provides a workaround; documented as a standing limitation in `CLAUDE.md` Rule 6 and `docs/LESSONS_LEARNED.md` §5 (a prior session found this same thing on 2026-08-01 but the finding was lost because the branch documenting it was itself never merged). The 17 safe branches need the repo owner's own GitHub web UI to actually delete.

Of the remaining 5 branches (real orphaned commits with no merged PR), investigation found: 3 (`feature/dvsc-onboarding`, `claude/knowledge-update-44cuix`, and most of `claude/salesleadgenerator-overview-aot320`) are artifacts of an earlier full-history rewrite (commit messages match `main` exactly, hashes don't) carrying zero genuinely unique content. `claude/salesleadgenerator-overview-aot320`'s one real unique commit is an admin tenant/queue-management webapp for the shared `researchandenrich`/OpenClaw ecosystem, co-authored by a different tool — correctly out of scope per Rule 9, safely abandoned. `chore/label-based-board-governance`'s one unique commit is an earlier, superseded attempt at project-board governance (a label taxonomy + `docs/PROJECT_BOARD.md`) — superseded by the `roadmap.md` decision, but its git-remote-403 finding is the one now recovered into this file.

`lead-taxonomy-loop`'s 8 unique commits (claimed issue #132 batches 17–23, "135 of ~2,723 leads") were spot-checked against live production: 1 of 3 checked leads' claimed fix demonstrably never landed (the real fix came a day later from an unrelated, independently-merged PR), a 2nd shows no classification at all, a 3rd's timing is ambiguous. Documented as a new lesson in `docs/LESSONS_LEARNED.md` §3 — an unmerged branch's own commit narrative is not proof of real production state.

Also: removed the local `.next` build directory (840MB, regenerated on demand) and this session's own scratch/debug files (~280MB) — no repo-tracked content affected.

### Testing
No app code changed — documentation-only. Full gate re-run: tsc 0 errors, lint 0 errors/warnings, vitest unit (702) passing.
