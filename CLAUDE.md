...
- **Standing instruction (owner, 2026-09-26): commit and push to `main` every time a deliverable chunk is finished, without waiting to be asked, and update the documentation in the same commit so no information is ever lost.** "Deliverable chunk" means anything that passes Rule 1 on its own: a shipped fix, a data batch, a doc correction, a new finding. Findings that exist only in a session's scratch files, a subagent's output or chat (investigation results, duplicate clusters found, owner-blocked steps, corrected premises) count as information that must be written to the repo docs or the relevant GitHub issue before the session moves on — scratch space and conversation history do not survive a session. Commits are authored under the owner's identity (`git config user.name moldovancsaba`, `user.email moldovancsaba@gmail.com`), never an AI identity (Rule 8).
