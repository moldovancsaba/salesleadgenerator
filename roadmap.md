...
| "Deliver the rest" dependency migration | Sequenced, in-progress effort (2.4.23+) covering the 9-package major-version backlog flagged in 2.4.22: integration tests done (2.4.23), TypeScript bumped to 6.0.3 with 7 explicitly blocked on upstream `typescript-eslint` support (2.4.24), React 18→19 done (2.4.25) — see `docs/STACK_AND_DEPENDENCIES.md`'s Dependency Audit table for the full sequence and current status of each remaining package (Next.js 16 + ESLint 10, Mantine 8→9, Mongoose 9) |
