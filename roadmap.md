...
| "Deliver the rest" dependency migration | Sequenced, in-progress effort (2.4.23+) covering the 9-package major-version backlog flagged in 2.4.22: integration tests done (2.4.23), TypeScript bumped to 6.0.3 with 7 explicitly blocked on upstream `typescript-eslint` support (2.4.24), React 18→19 done (2.4.25), Next.js 15→16 done with ESLint 10 attempted-and-reverted on a separate, also-blocked `typescript-eslint` issue (2.4.26), Mantine 7→9 done in a single jump after real 7→8 breaking-change research found nothing applicable (2.4.27) — see `docs/STACK_AND_DEPENDENCIES.md`'s Dependency Audit table for the full sequence; only Mongoose 8→9 remains |
